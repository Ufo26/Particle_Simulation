...
namespace UFO {
    /// <summary> Класс для хода выполнения процесса вычислений. </summary>
    public class TLoadProcess {
        /// <summary> Перечисление варианта завершения процесса вычислений в методе <b>Stop()</b>: <br/> <b>Show</b> - показать отчёт; <br/> <b>Hide</b> - скрыть отчёт; <br/> Отчёт показывает служебную информацию и общее время работы процессов. </summary>
        ...
        public /*async*/ void _Update() {
            ...
            PercentText.Text = "Выполнено: " +
                //Languages.TLoadProcess[1]/*Выполнено:*/ + 
                (int)((double)LoadProgressBar.Value / LoadProgressBar.Maximum * 100.0) + "%";
            LoadText.Left = (LoadPanel.Width - LoadText.Width) / 2;
            PercentText.Left = LoadProgressBar.Right - PercentText.Width;//лепим справа
            LoadPanel.Refresh();
        ...
        public void Init_Controls(Control Parent, Size PanelSize, Color PanelBackColor, string HeadText, Color TextForeColor) {
            LoadPanel = new Panel { Parent = Parent, Visible = false, BackColor = PanelBackColor, Size = PanelSize,
                Location = new Point((Parent.ClientSize.Width - PanelSize.Width) / 2, (Parent.ClientSize.Height - PanelSize.Height) / 2),
            };
            ...
            LoadText.Centering_X();// .Left = (LoadPanel.Width - LoadText.Width) / 2;//лепим по центру
            ...
            LoadProgressBar.Centering_X();// .Left = (LoadPanel.Width - LoadProgressBar.Width) / 2;//лепим по центру
        ...
        public void Stop(Report MB = Report.Hide) { 
            ...
            if (MB == Report.Show) { MessageBox.Show(
                    ...
                    "Прошло тиков: " + Time.ElapsedTicks + "\n\t" +
                    "Прошло милисек: " + Time.ElapsedMilliseconds + "\n\t" +
                    "Прошло сек: " + System.Math.Round(Time.Elapsed.TotalSeconds, 2) + "\n\t" +
                    "Время: " + Converter.msToTimeString((int)(Time.ElapsedMilliseconds)) + "\n" +
                    ...
                    //Languages.TLoadProcess[3] + " " +/*Прошло*/ Languages.TLoadProcess[6] + " "/*сек:*/ + System.Math.Round(Time.Elapsed.TotalSeconds, 2) + "\n\t" +
                    //Languages.TLoadProcess[7] + " " +/*Время:*/ Convert.msToTimeString((int)(Time.ElapsedMilliseconds)) + "\n" +
