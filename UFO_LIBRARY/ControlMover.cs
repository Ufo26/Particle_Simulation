            ...
            if (Math.Max(Math.Abs(dx), Math.Abs(dy)) > 1) {
                if (resizing) {
                    ...
                    if (AllowMove) {
                        ...
                        if (newLoc.X + ctrl.Width > GLOBAL_VAR.SCREEN.WorkingArea.Width)
                            newLoc = new Point(GLOBAL_VAR.SCREEN.WorkingArea.Width - ctrl.Width, newLoc.Y);
                        //запрет на перенос объекта за нижнюю границу экрана
                        if (newLoc.Y + ctrl.Height > GLOBAL_VAR.SCREEN.WorkingArea.Height)
                            newLoc = new Point(newLoc.X, GLOBAL_VAR.SCREEN.WorkingArea.Height - ctrl.Height);
